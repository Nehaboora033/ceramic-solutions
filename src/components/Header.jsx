  ...
  return (
    <>
      <div className='bg-center bg-cover bg-no-repeat py-6' style={{ backgroundImage: `url(${backgroundImage})` }}>
        ...
        <nav className={`fixed top-6 w-full ${scrolled ? '' : 'bg-transparent'}`}>
          <div className={`max-w-[1140px] mx-auto px-3 bg-white rounded-3xl p-4 flex items-center justify-between`}>
              ...
              {NAV.map((item, index) => {
                return (
                  <div key={index} className=''>
                    <li>
                      <a href="" className='font-normal text-base font-jakarta'>{item.name}</a>
                    </li>
                  </div>
                )
              })}
            ...
            <div className='flex items-center gap-3'>
              <button className='bg-[#FF0000] px-5 py-[14px] whitespace-nowrap  max-sm:w-[50px] max-sm:h-[50px] max-sm:p-0 max-sm:flex max-sm:items-center max-sm:justify-center max-w-[186px] rounded-[50%] sm:rounded-[56px] text-white font-bold text-base flex gap-2.5 cursor-pointer'>
                <CALL />
        ...
        <div className='max-w-[1140px] px-3 mx-auto pt-[180px]'>
          <h1 className='font-semibold md:text-[64px] text-[50px] text-white max-w-[830px] w-full  font-jakarta'>Transform Your Space with Precision and Quality:
            Get a Free Quote Today!</h1>
          <p className='font-normal md:text-lg text-base text-white max-w-[742px] mt-5 font-jakarta'>Lörem ipsum koda astrobel: sutaveligen. Rodod bänera viliga. Pregigt primasofi dede facebooka: förutom tivaligt. Fejkade nyheter eurosamma sol: teroktig,</p>
          ...
          <div className='flex gap-6  md:flex-row flex-col '>
            <div className='flex flex-row max-[500px]:flex-col gap-6'>
              {COMPANIES_REVIEW.map((item, index) => {
                return (
                  <div key={index} className='px-4 max-w-[229px] pt-[8.33px] pb-[6px]  bg-white rounded-2xl flex gap-[10px]'>
