            ...
            <div className='mx-w-[1140px] px-3 mx-auto mt-[140px]'>
                ...
                <Slider {...settings}>
                    {CUSTOMER_SLIDER.map((item, index) => (
                        <div key={index} className='p-6 max-w-[364px] rounded-3xl shadow-slider my-[56px]'>
                            <div>
                                <div className='flex gap-2'>
                                    <img src={item.image} alt="profile" />
                                    <div>
                                        <p className='font-semibold text-base'>{item.name} </p>
                                        <p className='font-normal text-sm'>{item.profession} </p>
                                    </div>
                                </div>
                                <img src={item.rating} alt="rating" className='my-4' />
                                <p className='font-normal text-base max-w-[316px]'>"{item.about}"</p>
                            </div>
                        </div>
                    ))}
                </Slider>
